---
name: triage
description: Classify an incoming code request and route it into the Killhouse pipeline. Use when work arrives raw — a bug report, a feature request, an external PR, an unscoped change — and you need to decide category, state, and how much process it warrants. Also moves issues through a state machine of triage roles, grilling and writing agent-ready briefs where needed.
---
...
Within the Killhouse pipeline, triage's job is the routing decision `ask-kh` consumes:
...
as the load-bearing output. A change that looks trivial but crosses a mandatory-gate boundary is not trivial —
re-classify as major. Return `classification: trivial | major` with a one-line rationale to `ask-kh`.
