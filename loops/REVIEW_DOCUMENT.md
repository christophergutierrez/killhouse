...
The reviewed document must contain only standard 7-bit US-ASCII (code points 0x00-0x7F; in practice the printable range 0x20-0x7E plus newline and tab). Never use extended or high ASCII (0x80-0xFF) or any other non-ASCII character. Replace, every round, any of the following with its ASCII equivalent:
...
This constraint is non-negotiable and survives every edit: if a revision introduces a non-ASCII character, the next round must remove it. Verify with `grep -nP '[^\x00-\x7F]' <file_path>` returning no matches.
...
**Default (in-session orchestration)**: The caller's own session plays the orchestrator. It launches the reviewer subagents in parallel, edits, and loops. Simple and fully parallel, but the caller's context grows with the work.

**Opt-in (delegated orchestration)**: The caller hands the entire loop to a single top-tier orchestrator subagent and gets back only a summary, so the caller's session stays lean. Use this for large documents, or when the caller is itself running inside a larger loop.
...
In every delegated run the orchestrator returns a lean result to the caller: a short changelog plus any open questions, not the full document.
...
- **Stop when**: All nine reviewers return "no issues" or equivalent, OR only style-preference-level feedback remains that the editor judges not worth changing.
- **Maximum loops**: 10. This is a hard cap, not a target: never exceed 10 rounds even if reviewers still have suggestions.
